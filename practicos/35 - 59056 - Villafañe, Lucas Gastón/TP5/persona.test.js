...
describe("Obtener una persona por ID", () => {
  test("1. Debe responder con un status 200 si la persona existe", async () => {
    const res = await request(app).get("/personas/1").send();
    expect(res.statusCode).toBe(200);
  });

  test("2. Debe retornar un JSON con los datos de la persona", async () => {
    const res = await request(app).get("/personas/1").send();
    expect(res.type).toBe("application/json");
    expect(res.body).toEqual(
      expect.objectContaining({
  ...
  test("3. Debe responder con un status 404 si la persona no existe", async () => {
    const res = await request(app).get("/personas/999").send();
    expect(res.statusCode).toBe(404);
  });
});
describe("Actualizar una persona por ID", () => {
  test("1. Actualizar una persona existente (retorna codigo)", async () => {
    const persona = { nombre: "Nuevo Nombre" };
    const res = await request(app).put("/personas/1").send(persona);
    expect(res.statusCode).toBe(201);
  });

  test("2. Si no existe la persona, no se actualiza", async () => {
    const persona = { nombre: "Nuevo Nombre" };
    const res = await request(app).put("/personas/999").send(persona);
    expect(res.statusCode).toBe(404);
  });

  test("3. Actualizar una persona (y retorna la persona actualizada)", async () => {
    const persona = { nombre: "Nuevo Nombre", edad: 40 };
    const res = await request(app).put("/personas/1").send(persona);
    expect(res.body.nombre).toBe(persona.nombre);
    expect(res.body.edad).toBe(persona.edad);
  });
});
